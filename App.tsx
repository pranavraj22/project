...
import { AlertCircle, Loader2, Map, Sparkles, ClipboardList, GraduationCap, Layers, Award } from 'lucide-react';
...
const App: React.FC = () => {
  const [view, setView] = useState<'exam' | 'plan' | 'clarifier' | 'mastery'>('exam');
    ...
    <div className="min-h-screen bg-bg flex flex-col print:bg-white relative">
      ...
      <main className="flex-grow flex flex-col items-center p-4 sm:p-6 w-full max-w-4xl mx-auto print:p-0 print:w-full print:max-w-none">
        ...
        {view === 'plan' ? (
          ...
          <SkillMasteryView onBack={() => setView('exam')} onLoadingChange={setLoading} />
        ) : (
          <>
            <div className={`w-full max-w-2xl bg-white rounded-3xl shadow-xl p-6 sm:p-8 transition-all duration-500 ease-out z-10 print:hidden ${result ? 'mb-8' : 'mb-0'}`}>
              ...
              <div className="space-y-4 mb-8">
                ...
                {/* Conditional Units Input for Mid Exam */}
                {details.examType === 'Mid Examination' && (
                  <div className="group animate-fade-in bg-blue-50/50 p-4 rounded-2xl border border-blue-100">
                    <label className="block text-sm font-semibold text-slate-700 mb-1 ml-1">
                      Target Units / Modules <span className="text-red-500">*</span>
                    </label>
                    <div className="relative">
                      <Layers className="absolute left-4 top-3.5 text-navy w-5 h-5 pointer-events-none" />
                      <input
                        type="text"
                        name="units"
                        value={details.units || ''}
                        onChange={handleInputChange}
                        placeholder="e.g. Unit 1 & 2, or Modules 1-3"
              ...
              <button onClick={() => setView('mastery')} disabled={loading} className="w-full h-[50px] mt-3 rounded-xl font-semibold text-navy border-2 border-navy/10 hover:border-navy hover:bg-blue-50 transition-all flex items-center justify-center gap-2 group text-sm sm:text-base">
                <Award size={18} className="text-neon group-hover:scale-110 transition-transform" />
                Skill Mastery
              </button>
            </div>
